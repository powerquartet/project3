...
class Grid extends Component {
  ...
  componentDidMount() {
    let createdPortions = this.state.portions
      .map((item, index) => {
  ...
  render() {
    ...
    const targets = this.state.meals.map((meal, indx) => {
      return (
        <Target
          portions={this.state.meals[indx].portions}
          meal={this.state.meals[indx].meal}
          key={meal.meal}
    ...
    return (
      <div className = "container">
        ...
        <div style={{ float: "left" }}>{vegetables}</div>
        <div style={{ float: "left" }}>{grains}</div>
        <div style={{ float: "left" }}>{protein}</div>
        <div style={{ float: "left" }}>{dairy}</div>
        <br style={{ clear: "both" }} />
        {targets}
      </div>
    );
